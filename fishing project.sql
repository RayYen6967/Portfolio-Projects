...
--Looking at weight, adjusted price, nao_index, surf_temp by year 
...
select avg(weight) as avg_weight, avg(value_cpi/weight) as avg_price,
       avg(nao_index) as avg_nao_index, avg(surf_temp) as avg_surf_temp,
	   count(ID) as total_vessels, datepart(yy, date) as year
from NorthAtlanticFishing
where fishing = 'Yes' and datepart(yy, date) != 2019
group by datepart(yy, date)
order by datepart(yy, date)

--Looking at weight, adjusted price, nao_index, surf_temp by month 
--Exclude 2019 because it only includes data from Jan.-June

select avg(weight) as avg_weight, avg(value_cpi/weight) as avg_price,
       avg(nao_index) as avg_nao_index, avg(surf_temp) as avg_surf_temp,
	   count(ID) as total_vessels, datepart(mm, date) as month
from NorthAtlanticFishing
where fishing = 'Yes' and datepart(yy, date) <> 2019
